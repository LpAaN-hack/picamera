...
<html>
<head>
<title>Welcome to CDU from IOT club. Join our club for exciting IOT and AI projects.</title>
...
# on the server calling and response capture
class StreamingHandler(server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            self.send_response(301)
            self.send_header('Location', '/index.html')  # image response from index page
            self.end_headers()
            ...
            with picamera.PiCamera(resolution='1280x720') as camera:
                camera.rotation = 180  # resolution with rotation angle of the camera
